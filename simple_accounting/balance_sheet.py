...
class BalanceSheet():
    ...
    def check_for_events(self):
        ...
        debited_events = self.event_store.pull_events(
            AccountDebited, self.__time_last_pulled)
        credited_events = self.event_store.pull_events(
            AccountCredited, self.__time_last_pulled)
        all_events = sorted(
            debited_events + credited_events,
            key=lambda event: event.timestamp)
        most_recent_events = list(
            {event.account_id: event for event in all_events}.values())
